...
module NicoPodcast
  ...
  class Video < Item
    ...
    def encode
      ...
      (system "ffmpeg -i #{self.original_path} -f mp4 -acodec libfaac -vcodec libx264 -b 512k -ac 2 -ar 48000 -ab 96k -vpre default  #{self.path('mp4')} >& /dev/null" or File.unlink(self.path('mp4'))) unless File.exist?(self.path('mp4'))
    end
